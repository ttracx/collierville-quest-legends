...
export const generateAvatar = (canvas: HTMLCanvasElement, character: 'mike' | 'carson' | 'ava') => {
...
const drawCarsonAvatar = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
  ...
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.arc(centerX - 18, centerY - 25, 12, Math.PI * 1.2, Math.PI * 1.8);
  ctx.arc(centerX + 18, centerY - 25, 12, Math.PI * 1.2, Math.PI * 1.8);
  ctx.stroke();
...
export const generateAvatarDataURL = (character: 'mike' | 'carson' | 'ava'): string => {
